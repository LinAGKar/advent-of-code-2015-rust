...
fn calc(
    ...
    if pos == ingredients.len() - 1 {
        if *totals.last().unwrap() as u16 + *ingredients[pos].last().unwrap() as u16 * remaining as u16 != 500 {
            i32::MIN
        } else {
            totals.iter().take(totals.len() - 1).zip(ingredients[pos].iter()).map(|(&a, &b)| {
...
fn main() {
    ...
    let ingredients: Vec<Vec<i8>> = input.lines().map(|line| {
        ...
        [2, 1, 1, 1, 1].iter().map(|&n| words.nth(n).unwrap().trim_matches(',').parse().unwrap()).collect()
    }).collect();
