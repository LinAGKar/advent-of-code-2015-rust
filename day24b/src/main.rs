...
fn main() {
    ...
    for i in 0..weights.len() {
        ...
        loop {
            ...
            if group_total == total / 4 {
                ...
                if (1..=remaining_weights.len()).any(|count| {
                    ...
                    loop {
                        ...
                        if group_total == total / 4 {
                            let remaining_weights: Vec<_> = remaining_weights.iter().take(taken[0]).cloned().chain(
                                taken.windows(2).flat_map(|j| {
                                    &weights[j[0] + 1..j[1]]
                                }).cloned()
                            ).chain(remaining_weights.iter().skip(*taken.last().unwrap() + 1).cloned()).collect();
                            if (1..=remaining_weights.len()).any(|count| {
                                let mut taken: Vec<_> = (0..count).collect();
                                loop {
                                    let group_total: u32 = taken.iter().map(|&x| remaining_weights[x]).sum();
                                    ...
                                    if !increment(&mut taken, group_total <= total / 4, remaining_weights.len()) {
                                        break;
                                    }
                                }
                                false
            ...
            if !increment(&mut taken, group_total <= total / 4, weights.len()) {
                break;
            }
        }
